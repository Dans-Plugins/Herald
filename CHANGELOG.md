...
- Startup now warns when email notifications are partially configured, naming each missing key (`email-recipients`, `smtp.server`, `email.sender`) instead of logging `Email notifications enabled` and then failing on every player join.
