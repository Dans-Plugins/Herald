...
- Bug report and feature request issue templates under `.github/ISSUE_TEMPLATE/`, a `CODEOWNERS` file, and a "Commit and Pull Request Conventions" section in `CONTRIBUTING.md` documenting the branch prefixes, commit-message style, `Closes #N` linking, and squash-merge strategy already in use. The README installation step now links to the releases page.
