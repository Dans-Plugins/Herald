...
- `Email configuration is incomplete: ...` — at least one of `email-recipients`, `smtp.server` and `email.sender` is filled in, but a required key is missing, `smtp.port` is outside the valid range, or an address in `email-recipients` or `email.sender` cannot be parsed; the message names each missing or invalid key, and quotes the offending address where one is at fault.
